...
/*	      	Node *temp=new Node(el);   Made a node and passing value to it
		  ...
		  bool Search(int el)
		  {
		  	Node *current,*prev,*preprev,*temp;
		  	for(current=head,preprev=prev=head;current!=0;preprev=prev,prev=current,current=current->next)
		  	{
		  		if(current->data==el)
		  		{
		  			if(current==tail)
				  	...
				  	//SwapWithPrevious();
				  	preprev->next=current;
				  	prev->next=current->next;
				  	current->next=prev;
				  	
				  /*	int val=current->data;		(if mam say swap only value)
